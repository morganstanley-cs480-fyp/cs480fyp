...
@pytest.mark.asyncio
async def test_resolve_exception_disabled_does_not_update_status(
    client: AsyncClient, sample_exception
):
    """Resolution endpoint exists but is disabled and must not mutate data."""
    assert sample_exception.status == "PENDING"

    response = await client.post(f"/api/exceptions/{sample_exception.id}/resolve")

    assert response.status_code == 403
    assert response.json()["detail"] == "Exception resolution is disabled"
...
async def test_resolve_exception_not_found(client: AsyncClient):
    """Resolving a non-existent exception still returns 404."""
