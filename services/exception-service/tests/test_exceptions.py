...
async def test_update_exception(client: AsyncClient, sample_exception):
    ...
    payload = {"msg": "Updated message", "priority": "Critical", "status": "CLOSED"}
...
@pytest.mark.asyncio
async def test_get_exceptions_by_trade_with_sample(
    client: AsyncClient, sample_exception
):
...
@pytest.mark.asyncio
async def test_resolve_exception_is_disabled_for_pending_exception(
    client: AsyncClient, sample_exception
):
    """Test resolving an exception is blocked and does not change status."""
    assert sample_exception.status == "PENDING"

    response = await client.post(f"/api/exceptions/{sample_exception.id}/resolve")

    assert response.status_code == 403
    assert response.json()["detail"] == "Exception resolution is currently disabled for users"
...
@pytest.mark.asyncio
async def test_resolve_exception_is_disabled_for_closed_exception(client: AsyncClient):
    """Test resolving a CLOSED exception is also blocked."""
    exception = await Exception.create(
        trade_id=7777,
        trans_id=77777,
    ...
    await exception.refresh_from_db()
...
@pytest.mark.asyncio
async def test_resolve_exception_is_disabled_even_when_not_found(client: AsyncClient):
    """Test resolving a non-existent exception is blocked by feature disablement check."""
