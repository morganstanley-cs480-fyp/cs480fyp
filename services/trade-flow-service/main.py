...
async def get_transaction_by_id(id: int):
  try:
    async with app.state.pool.connection() as conn:
      async with conn.cursor(row_factory=dict_row) as cur:
        await cur.execute("SELECT * FROM transactions WHERE id=%s", (id,))
...
async def get_transactions_by_trade_id(trade_id: int):
  try:
    async with app.state.pool.connection() as conn:
      async with conn.cursor(row_factory=dict_row) as cur:
        await cur.execute("SELECT * FROM transactions WHERE trade_id=%s", (trade_id,))
        transactions = await cur.fetchall()
        
        if not transactions:
          raise HTTPException(status_code=404, detail=f"Transctions with trade_id {trade_id} not found")
